...
const ProductsComponent = () => {
  ...
  useEffect(() => {
    getCategories().then((response: string[]) => setCategories(response));
  }, []);

  useEffect(() => {
    if (selectedCategory) {
      getProductsByCategory(selectedCategory).then((fetchedProducts: Product[]) =>
        setProducts(fetchedProducts)
      );
    } else {
      getAllProducts().then((fetchedProducts: Product[]) => setProducts(fetchedProducts));
    }
  }, [selectedCategory]);
